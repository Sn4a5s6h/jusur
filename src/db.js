...
try {
    ...
    if (!company) {
        db.prepare(`INSERT INTO companies (name, legal_name, currency) VALUES (?, ?, ?)`)
            .run("الشركة الافتراضية", "الشركة الافتراضية", "YER");
...
try {
    ...
    if (!admin) {
        ...
        db.prepare(`
            INSERT INTO users (username, password_hash, name, full_name, role, status, active, company_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run("admin", hashedPassword, "مدير النظام", "مدير النظام", "admin", "active", 1, company ? company.id : null);
